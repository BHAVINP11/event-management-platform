...
exports.loadActiveOrganizationMembership = loadActiveOrganizationMembership;
exports.verifyOrganizationManagementAuthority = verifyOrganizationManagementAuthority;
 ...
 */
async function loadActiveOrganizationMembership(db, organizationId, userId) {
 ...
 */
async function verifyOrganizationManagementAuthority(db, organizationId, userId) {
    ...
    if (!membership.role || !ORGANIZATION_MANAGEMENT_ROLES.includes(membership.role)) {
        throw new validation_1.ValidationError('organization_role_not_allowed', 'Your role does not allow managing this organization.');
    }
}
