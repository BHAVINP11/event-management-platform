...
describe('mapErrorToCallableResponse', () => {
    ...
    test('preserves the original message alongside the mapped code', () => {
        const result = (0, errorMapping_1.mapErrorToCallableResponse)(new validation_1.ValidationError('organization_role_not_allowed', 'Your role does not allow this.'));
    ...
    test('maps an unrecognized ValidationError code to internal, not invalid-argument', () => {
        const result = (0, errorMapping_1.mapErrorToCallableResponse)(new validation_1.ValidationError('something_new', 'message'));
    ...
    test('maps a Firestore "already exists" error without leaking its message', () => {
        const result = (0, errorMapping_1.mapErrorToCallableResponse)(new Error('6 ALREADY_EXISTS: document already exists'));
        expect(result).toEqual({
    ...
    test('maps a Firestore permission error without leaking its message', () => {
        const result = (0, errorMapping_1.mapErrorToCallableResponse)(new Error('7 PERMISSION_DENIED: Missing permissions'));
        expect(result).toEqual({
            firebaseCode: 'permission-denied',
    ...
    test('maps an unrecognized Error to a generic internal error, never its raw message', () => {
        const result = (0, errorMapping_1.mapErrorToCallableResponse)(new Error('Firestore: 5 NOT_FOUND at internal/path/xyz'));
        expect(result).toEqual({
    ...
    test('maps a non-Error throw to a generic internal error', () => {
        const result = (0, errorMapping_1.mapErrorToCallableResponse)('a raw string throw');
        expect(result).toEqual({
            firebaseCode: 'internal',
            message: 'An unexpected error occurred. Please try again.',
            appCode: 'internal_error'
