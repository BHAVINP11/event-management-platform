...
describe('handleCreateExpense', () => {
    test('rejects an unauthenticated request', async () => {
        ...
        await expect((0, createExpense_1.handleCreateExpense)(db, validInput, {})).rejects.toMatchObject({ code: 'unauthenticated' });
    });
    test('a missing event is reported as not found', async () => {
        const db = (0, fakeFirestore_1.asFirestore)(new fakeFirestore_1.FakeFirestore());
        await expect((0, createExpense_1.handleCreateExpense)(db, validInput, { auth: { uid: 'user1' } })).rejects.toMatchObject({
    ...
    test('an owner can create an expense', async () => {
        ...
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        const result = await (0, createExpense_1.handleCreateExpense)(db, validInput, { auth: { uid: 'user1' } });
    ...
    test('rejects a missing title', async () => {
        ...
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        await expect((0, createExpense_1.handleCreateExpense)(db, { ...validInput, title: '' }, { auth: { uid: 'user1' } })).rejects.toBeInstanceOf(validation_1.ValidationError);
    });
    test('rejects an invalid category', async () => {
        ...
        seedEventMember(fake, 'user1');
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        await expect((0, createExpense_1.handleCreateExpense)(db, { ...validInput, category: 'flowers' }, { auth: { uid: 'user1' } })).rejects.toMatchObject({ code: 'invalid_category' });
    });
    test.each([0, -100])('rejects a non-positive amount (%d)', async (amount) => {
        ...
        seedEventMember(fake, 'user1');
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        await expect((0, createExpense_1.handleCreateExpense)(db, { ...validInput, amount }, { auth: { uid: 'user1' } })).rejects.toMatchObject({ code: 'invalid_amount' });
    });
    test('rejects an invalid payment status', async () => {
        ...
        seedEventMember(fake, 'user1');
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        await expect((0, createExpense_1.handleCreateExpense)(db, { ...validInput, paymentStatus: 'overdue' }, { auth: { uid: 'user1' } })).rejects.toMatchObject({ code: 'invalid_payment_status' });
    });
    test('rejects an invalid payment date', async () => {
        ...
        seedEventMember(fake, 'user1');
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        await expect((0, createExpense_1.handleCreateExpense)(db, { ...validInput, paymentDate: 'not-a-date' }, { auth: { uid: 'user1' } })).rejects.toMatchObject({ code: 'invalid_payment_date' });
    });
    ...
    test('a fully paid expense gets paidAmount equal to amount, regardless of client input', async () => {
        ...
        seedEventMember(fake, 'user1');
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        const result = await (0, createExpense_1.handleCreateExpense)(db, { ...validInput, paymentStatus: 'paid', paidAmount: 1 }, { auth: { uid: 'user1' } });
    ...
    test('a partially paid expense requires and validates paidAmount', async () => {
        ...
        seedEventMember(fake, 'user1');
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        const result = await (0, createExpense_1.handleCreateExpense)(db, { ...validInput, paymentStatus: 'partially_paid', paidAmount: 100000 }, { auth: { uid: 'user1' } });
        expect(fake.read('expenses', result.expenseId)).toMatchObject({
    ...
    test('rejects a partially paid expense with no paidAmount', async () => {
        ...
        seedEventMember(fake, 'user1');
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        await expect((0, createExpense_1.handleCreateExpense)(db, { ...validInput, paymentStatus: 'partially_paid' }, { auth: { uid: 'user1' } })).rejects.toMatchObject({ code: 'invalid_paid_amount' });
    });
    test('rejects a negative paidAmount for a partially paid expense', async () => {
        ...
        seedEventMember(fake, 'user1');
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        await expect((0, createExpense_1.handleCreateExpense)(db, { ...validInput, paymentStatus: 'partially_paid', paidAmount: -1 }, { auth: { uid: 'user1' } })).rejects.toMatchObject({ code: 'invalid_paid_amount' });
    });
    test('rejects a paidAmount greater than the expense amount', async () => {
        ...
        seedEventMember(fake, 'user1');
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        await expect((0, createExpense_1.handleCreateExpense)(db, { ...validInput, paymentStatus: 'partially_paid', paidAmount: 300000 }, { auth: { uid: 'user1' } })).rejects.toMatchObject({ code: 'invalid_paid_amount' });
    });
    test('an unpaid expense forces paidAmount to 0, regardless of client input', async () => {
        ...
        seedEventMember(fake, 'user1');
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        const result = await (0, createExpense_1.handleCreateExpense)(db, { ...validInput, paymentStatus: 'unpaid', paidAmount: 999999 }, { auth: { uid: 'user1' } });
    ...
    test('createdBy comes from the authenticated UID, not client input', async () => {
        ...
        seedEventMember(fake, 'user1');
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        const result = await (0, createExpense_1.handleCreateExpense)(db, { ...validInput, createdBy: 'someone-else', id: 'chosen-by-client' }, { auth: { uid: 'user1' } });
