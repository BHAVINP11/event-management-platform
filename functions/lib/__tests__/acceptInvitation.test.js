...
describe('handleAcceptInvitation', () => {
    test('rejects an unauthenticated request', async () => {
        ...
        await expect((0, acceptInvitation_1.handleAcceptInvitation)(db, acceptInput, {})).rejects.toMatchObject({
            code: 'unauthenticated'
        });
    });
    test('rejects an invitation that does not exist', async () => {
        const db = (0, fakeFirestore_1.asFirestore)(new fakeFirestore_1.FakeFirestore());
        await expect((0, acceptInvitation_1.handleAcceptInvitation)(db, acceptInput, { auth: { uid: 'user1', token: { email: 'priya@example.com' } } })).rejects.toMatchObject({ code: 'invitation_not_found' });
    });
    test('rejects an invitation that is no longer pending', async () => {
        ...
        await expect((0, acceptInvitation_1.handleAcceptInvitation)(db, acceptInput, { auth: { uid: 'user1', token: { email: 'priya@example.com' } } })).rejects.toMatchObject({ code: 'invitation_not_pending' });
    });
    test('rejects an expired invitation', async () => {
        const fake = new fakeFirestore_1.FakeFirestore();
        seedInvitation(fake, { expiresAt: past });
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        await expect((0, acceptInvitation_1.handleAcceptInvitation)(db, acceptInput, { auth: { uid: 'user1', token: { email: 'priya@example.com' } } })).rejects.toMatchObject({ code: 'invitation_expired' });
    });
    test("rejects a caller whose email does not match the invitation's", async () => {
        ...
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        await expect((0, acceptInvitation_1.handleAcceptInvitation)(db, acceptInput, { auth: { uid: 'user1', token: { email: 'someone-else@example.com' } } })).rejects.toMatchObject({ code: 'invitation_email_mismatch' });
    });
    test('rejects a caller with no email on their auth token', async () => {
        const fake = new fakeFirestore_1.FakeFirestore();
        seedInvitation(fake);
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        await expect((0, acceptInvitation_1.handleAcceptInvitation)(db, acceptInput, { auth: { uid: 'user1' } })).rejects.toMatchObject({ code: 'invitation_email_mismatch' });
    });
    test('a valid invitation creates the EventMember with the deterministic ID', async () => {
        const fake = new fakeFirestore_1.FakeFirestore();
        seedInvitation(fake);
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        const result = await (0, acceptInvitation_1.handleAcceptInvitation)(db, acceptInput, {
    ...
    test('the membership has the role and side copied from the invitation', async () => {
        ...
        const db = (0, fakeFirestore_1.asFirestore)(fake);
        await (0, acceptInvitation_1.handleAcceptInvitation)(db, acceptInput, {
            auth: { uid: 'user1', token: { email: 'priya@example.com' } }
        });
