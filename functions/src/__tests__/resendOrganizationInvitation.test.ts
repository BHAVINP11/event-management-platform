...
function seedInvitation(
  ...
  overrides: { organizationId?: string; status?: string; expiresAt?: string } = {}
): void {
...
describe('handleResendOrganizationInvitation', () => {
  test('rejects an unauthenticated request', async () => {
    ...
    await expect(handleResendOrganizationInvitation(db, validInput, {})).rejects.toMatchObject({
  ...
  test('a missing invitation is reported as not found', async () => {
    ...
    await expect(
      handleResendOrganizationInvitation(db, validInput, { auth: { uid: 'user1' } })
  ...
  test('an owner can resend a pending invitation, extending its expiry', async () => {
    ...
    const result = await handleResendOrganizationInvitation(db, validInput, { auth: { uid: 'user1' } });
  ...
  test('an admin can resend a pending invitation', async () => {
    ...
    await expect(handleResendOrganizationInvitation(db, validInput, { auth: { uid: 'user1' } })).resolves.toMatchObject({
      invitationId: INVITATION_ID
    });
  });
  ...
  test('does not affect an unrelated pending invitation for the same organization', async () => {
    ...
    await handleResendOrganizationInvitation(db, { invitationId: 'inv1' }, { auth: { uid: 'user1' } });
