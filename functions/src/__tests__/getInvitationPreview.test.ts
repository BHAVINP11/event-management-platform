...
function seedEvent(fake: FakeFirestore): void {
  fake.seed('events', EVENT_ID, { id: EVENT_ID, name: 'Bhavin & Priya Wedding' });
}

function seedInvitation(fake: FakeFirestore, overrides: Record<string, unknown> = {}): void {
...
describe('handleGetInvitationPreview', () => {
  test('rejects an unauthenticated request', async () => {
    ...
    await expect(handleGetInvitationPreview(db, previewInput, {})).rejects.toMatchObject({
  ...
  test('returns the event name and invitation summary for the invited person', async () => {
    ...
    const result = await handleGetInvitationPreview(db, previewInput, {
  ...
  test('rejects a caller whose email does not match the invitation', async () => {
    ...
    await expect(
      handleGetInvitationPreview(db, previewInput, {
